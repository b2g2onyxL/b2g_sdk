...
XPCOMUtils.defineLazyGetter(this,"gApp",function bls_gApp(){let appinfo=Cc["@mozilla.org/xre/app-info;1"].getService(Ci.nsIXULRuntime);try{appinfo.QueryInterface(Ci.nsIXULAppInfo);}catch(ex if ex instanceof Components.Exception&&ex.result==Cr.NS_NOINTERFACE){}
return appinfo;});XPCOMUtils.defineLazyGetter(this,"gABI",function bls_gABI(){let abi=null;try{abi=gApp.XPCOMABI;}
catch(e){LOG("BlockList Global gABI: XPCOM ABI unknown.");}
return abi;});XPCOMUtils.defineLazyGetter(this,"gOSVersion",function bls_gOSVersion(){let osVersion;let sysInfo=Cc["@mozilla.org/system-info;1"].getService(Ci.nsIPropertyBag2);try{osVersion=sysInfo.getProperty("name")+" "+sysInfo.getProperty("version");}
...
return Ci.nsIBlocklistService.STATE_NOT_BLOCKED;if(!appVersion&&!gApp.version)
...
pingCountTotal=1;dsURI=dsURI.replace(/%APP_ID%/g,gApp.ID);if(gApp.version)
dsURI=dsURI.replace(/%APP_VERSION%/g,gApp.version);dsURI=dsURI.replace(/%PRODUCT%/g,gApp.name);if(gApp.version)
dsURI=dsURI.replace(/%VERSION%/g,gApp.version);dsURI=dsURI.replace(/%BUILD_ID%/g,gApp.appBuildID);dsURI=dsURI.replace(/%BUILD_TARGET%/g,gApp.OS+"_"+gABI);dsURI=dsURI.replace(/%OS_VERSION%/g,gOSVersion);dsURI=dsURI.replace(/%LOCALE%/g,getLocale());dsURI=dsURI.replace(/%CHANNEL%/g,UpdateChannel.get());dsURI=dsURI.replace(/%PLATFORM_VERSION%/g,gApp.platformVersion);dsURI=dsURI.replace(/%DISTRIBUTION%/g,getDistributionPrefValue(PREF_APP_DISTRIBUTION));dsURI=dsURI.replace(/%DISTRIBUTION_VERSION%/g,getDistributionPrefValue(PREF_APP_DISTRIBUTION_VERSION));dsURI=dsURI.replace(/%PING_COUNT%/g,pingCountVersion);dsURI=dsURI.replace(/%TOTAL_PING_COUNT%/g,pingCountTotal);dsURI=dsURI.replace(/%DAYS_SINCE_LAST_PING%/g,daysSinceLastPing);dsURI=dsURI.replace(/\+/g,"%2B");
...
continue;switch(element.localName){case"emItems":this._addonEntries=this._processItemNodes(element.childNodes,"em",this._handleEmItemNode);break;case"pluginItems":this._pluginEntries=this._processItemNodes(element.childNodes,"plugin",this._handlePluginItemNode);break;case"certItems":this._processItemNodes(element.childNodes,"cert",this._handleCertItemNode.bind(this));break;default:Services.obs.notifyObservers(element,"blocklist-data-"+element.localName,null);}}
...
return result;},_handleCertItemNode:function Blocklist_handleCertItemNode(blocklistElement,result){let issuer=blocklistElement.getAttribute("issuerName");for(let snElement of blocklistElement.children){try{gCertBlocklistService.addRevokedCert(issuer,snElement.textContent);}catch(e){
 LOG("Blocklist::_handleCertItemNode: Error adding revoked cert "+e);}}},_handleEmItemNode:function Blocklist_handleEmItemNode(blocklistElement,result){if(!matchesOSABI(blocklistElement))
...
return;var matchNodes=blocklistElement.childNodes;var blockEntry={matches:{},versions:[],blockID:null,infoURL:null,};var hasMatch=false;for(var x=0;x<matchNodes.length;++x){var matchElement=matchNodes.item(x);if(!(matchElement instanceof Ci.nsIDOMElement))
...
if(matchElement.localName=="versionRange"){blockEntry.versions.push(new BlocklistItemData(matchElement));}
else if(matchElement.localName=="infoURL"){blockEntry.infoURL=matchElement.textContent;}} 
if(!hasMatch)
...
this._loadBlocklist();return this._getPluginBlocklistState(plugin,this._pluginEntries,appVersion,toolkitVersion);},_getPluginBlocklistEntry:function Blocklist_getPluginBlocklistEntry(plugin,pluginEntries,appVersion,toolkitVersion){if(!gBlocklistEnabled)
return null;if(!appVersion&&!gApp.version)
return Ci.nsIBlocklistService.STATE_NOT_BLOCKED;if(!appVersion)
appVersion=gApp.version;if(!toolkitVersion)
...
if(matchFailed)
continue;for(let blockEntryVersion of blockEntry.versions){if(blockEntryVersion.includesItem(plugin.version,appVersion,toolkitVersion)){return{entry:blockEntry,version:blockEntryVersion};}}}
return null;},_getPluginBlocklistState:function Blocklist_getPluginBlocklistState(plugin,pluginEntries,appVersion,toolkitVersion){let r=this._getPluginBlocklistEntry(plugin,pluginEntries,appVersion,toolkitVersion);if(!r){return Ci.nsIBlocklistService.STATE_NOT_BLOCKED;}
let{entry:blockEntry,version:blockEntryVersion}=r;if(blockEntryVersion.severity>=gBlocklistLevel)
...
return Ci.nsIBlocklistService.STATE_SOFTBLOCKED;},getPluginBlocklistURL:function Blocklist_getPluginBlocklistURL(plugin){if(!this._isBlocklistLoaded())
this._loadBlocklist();let r=this._getPluginBlocklistEntry(plugin,this._pluginEntries);if(!r){return null;}
let{entry:blockEntry,version:blockEntryVersion}=r;if(!blockEntry.blockID){return null;}
return this._createBlocklistURL(blockEntry.blockID);},getPluginInfoURL:function(plugin){if(!this._isBlocklistLoaded())
this._loadBlocklist();let r=this._getPluginBlocklistEntry(plugin,this._pluginEntries);if(!r){return null;}
let{entry:blockEntry,version:blockEntryVersion}=r;if(!blockEntry.blockID){return null;}
return blockEntry.infoURL;},_blocklistUpdated:function Blocklist_blocklistUpdated(oldAddonEntries,oldPluginEntries){var addonList=[];function resetPrefs(prefs){for(let pref of prefs)
