...
this._dispatchers={};this._pendings={};this._registerManifestURLReady=false;this._isHandling=false;this._promise=null;let appInfo=Cc["@mozilla.org/xre/app-info;1"];this._isParentProcess=!appInfo||appInfo.getService(Ci.nsIXULRuntime).processType==Ci.nsIXULRuntime.PROCESS_TYPE_DEFAULT;if(this._isParentProcess){Services.obs.addObserver(this,kSystemMessageInternalReady,false);}}
SystemMessageManager.prototype={__proto__:DOMRequestIpcHelper.prototype,_dispatchMessage:function(aType,aDispatcher,aMessage,aMessageID){if(aDispatcher.isHandling){


aDispatcher.messages.push({message:aMessage,messageID:aMessageID});return;}
aDispatcher.isHandling=true;this._isHandling=true;
...
let message=wrapped?aMessage:Cu.cloneInto(aMessage,this._window);let rejectPromise=false;try{aDispatcher.handler.handleMessage(message);}catch(e){rejectPromise=true;}
this._isHandling=false;aDispatcher.isHandling=false;let self=this;function sendResponse(){

cpmm.sendAsyncMessage("SystemMessageManager:HandleMessageDone",{type:aType,manifestURL:self._manifestURL,pageURL:self._pageURL,msgID:aMessageID,rejected:rejectPromise});}
if(!this._promise){debug("No promise set, sending the response immediately.");sendResponse();}else{debug("Using the promise to postpone the response.");this._promise.then(sendResponse,function(){rejectPromise=true;sendResponse();});this._promise=null;}
if(aDispatcher.messages.length>0){let msg=aDispatcher.messages.shift();this._dispatchMessage(aType,aDispatcher,msg.message,msg.messageID);}else{
...
let cache=Cc["@mozilla.org/system-message-cache;1"].getService(Ci.nsISystemMessageCache);return cache.hasPendingMessage(aType,this._pageURL,this._manifestURL);},mozIsHandlingMessage:function(){debug("is handling message: "+this._isHandling);return this._isHandling;},mozSetMessageHandlerPromise:function(aPromise){debug("setting a promise");if(!this._isHandling){throw"Not in a handleMessage method";}
if(this._promise){throw"Promise already set";}
this._promise=aPromise;},uninit:function(){this._dispatchers=null;this._pendings=null;this._promise=null;if(this._isParentProcess){Services.obs.removeObserver(this,kSystemMessageInternalReady);}
...
let messages=(aMessage.name=="SystemMessageManager:Message")?[{msg:msg.msg,msgID:msg.msgID}]:msg.msgQueue;let dispatcher=this._dispatchers[msg.type];if(dispatcher){if(aMessage.name=="SystemMessageManager:Message"){
...
messages.forEach(function(aMsg){this._dispatchMessage(msg.type,dispatcher,aMsg.msg,aMsg.msgID);},this);}else{
