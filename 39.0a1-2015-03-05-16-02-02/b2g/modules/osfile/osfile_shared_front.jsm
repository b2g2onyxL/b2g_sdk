...
throw OS.File.Error.closed("accessing file",this._path);},read:function read(maybeBytes,options={}){if(typeof maybeBytes==="object"){options=clone(maybeBytes);maybeBytes=null;}else{options=options||{};}
let bytes=options.bytes||undefined;if(bytes===undefined){bytes=maybeBytes==null?this.stat().size:maybeBytes;}
let buffer=new Uint8Array(bytes);let pos=0;while(pos<bytes){let length=bytes-pos;let view=new DataView(buffer.buffer,pos,length);let chunkSize=this._read(view,length,options);if(chunkSize==0){break;}
pos+=chunkSize;}
if(pos==bytes){return buffer;}else{return buffer.subarray(0,pos);}},write:function write(buffer,options={}){let bytes=SharedAll.normalizeBufferArgs(buffer,("bytes"in options)?options.bytes:undefined);let pos=0;while(pos<bytes){let length=bytes-pos;let view=new DataView(buffer.buffer,buffer.byteOffset+pos,length);let chunkSize=this._write(view,length,options);pos+=chunkSize;}
...
let decoder;try{decoder=new TextDecoder(options.encoding);}catch(ex if ex instanceof RangeError){throw OS.File.Error.invalidArgument("Decode");}
...
OS.File.removeEmptyDir(path);};AbstractFile.makeDir=function(path,options={}){let from=options.from;if(!from){OS.File._makeDir(path,options);return;}
if(!path.startsWith(from)){
path=Path.normalize(path);from=Path.normalize(from);if(!path.startsWith(from)){throw new Error("Incorrect use of option |from|: "+path+" is not a descendant of "+from);}}
let innerOptions=Object.create(options,{ignoreExisting:{value:true}});let items=Path.split(path).components.slice(Path.split(from).components.length);let current=from;for(let item of items){current=Path.join(current,item);OS.File._makeDir(current,innerOptions);}};if(!exports.OS.Shared){exports.OS.Shared={};}
