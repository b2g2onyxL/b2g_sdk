...
{this.owner=aOwner;this.receivedData="";this.httpActivity=aHttpActivity;this.bodySize=0;let channel=this.httpActivity.channel;this._wrappedNotificationCallbacks=channel.notificationCallbacks;channel.notificationCallbacks=this;}
...
try{let impl=this._wrappedNotificationCallbacks.getInterface(iid);impl[method].apply(impl,args);}catch(e if e.result==Cr.NS_ERROR_NO_INTERFACE){}},_foundOpenResponse:false,_wrappedNotificationCallbacks:null,owner:null,sink:null,httpActivity:null,receivedData:null,bodySize:null,transferredSize:null,request:null,setAsyncListener:function NRL_setAsyncListener(aStream,aListener)
...
{this.request=aRequest;this._getSecurityInfo();this._findOpenResponse();this.setAsyncListener(this.sink.inputStream,this);},_getSecurityInfo:DevToolsUtils.makeInfallible(function NRL_getSecurityInfo(){


let secinfo=this.httpActivity.channel.securityInfo;let info=NetworkHelper.parseSecurityInfo(secinfo,this.httpActivity);this.httpActivity.owner.addSecurityInfo(info);}),onStopRequest:function NRL_onStopRequest()
{this._findOpenResponse();this.sink.outputStream.close();}, onProgress:function(request,context,progress,progressMax){this.transferredSize=progress;
this._forwardNotification(Ci.nsIProgressEventSink,"onProgress",arguments);},onStatus:function(){this._forwardNotification(Ci.nsIProgressEventSink,"onStatus",arguments);},_findOpenResponse:function NRL__findOpenResponse()
...
{let response={mimeType:"",text:aData||"",};response.size=response.text.length;response.transferredSize=this.transferredSize;try{response.mimeType=this.request.contentType;}
catch(ex){}
...
this.receivedData="";this.httpActivity.owner.addResponseContent(response,this.httpActivity.discardResponseBody);this._wrappedNotificationCallbacks=null;this.httpActivity.channel=null;this.httpActivity.owner=null;this.httpActivity=null;this.sink=null;this.inputStream=null;this.request=null;this.owner=null;},onInputStreamReady:function NRL_onInputStreamReady(aStream)
...
if(this.window){
let win=NetworkHelper.getWindowForRequest(aChannel);while(win){if(win==this.window){return true;}
if(win.parent==win){break;}
win=win.parent;}}
if(aChannel.loadInfo){if(aChannel.loadInfo.contentPolicyType==Ci.nsIContentPolicy.TYPE_BEACON){return true;}}
...
httpActivity.owner=this.owner.onNetworkEvent(event,aChannel,this);this._setupResponseListener(httpActivity);this.openRequests[httpActivity.id]=httpActivity;httpActivity.owner.addRequestHeaders(headers,aExtraStringData);httpActivity.owner.addRequestCookies(cookies);},createActivityObject:function NM_createActivityObject(aChannel)
{return{id:gSequenceId(),channel:aChannel,charset:null,url:aChannel.URI.spec,hostname:aChannel.URI.host, discardRequestBody:!this.saveRequestAndResponseBodies,discardResponseBody:!this.saveRequestAndResponseBodies,timings:{},responseStatus:null,owner:null,};},_setupResponseListener:function NM__setupResponseListener(aHttpActivity)
...
let headers=aExtraStringData.split(/\r\n|\n|\r/);let statusLine=headers.shift();let statusLineArray=statusLine.split(" ");let response={};response.httpVersion=statusLineArray.shift();response.remoteAddress=aHttpActivity.channel.remoteAddress;response.remotePort=aHttpActivity.channel.remotePort;response.status=statusLineArray.shift();response.statusText=statusLineArray.join(" ");response.headersSize=aExtraStringData.length;aHttpActivity.responseStatus=response.status;switch(parseInt(response.status)){case HTTP_MOVED_PERMANENTLY:case HTTP_FOUND:case HTTP_SEE_OTHER:case HTTP_TEMPORARY_REDIRECT:aHttpActivity.discardResponseBody=true;break;}
response.discardResponseBody=aHttpActivity.discardResponseBody;aHttpActivity.owner.addResponseStart(response,aExtraStringData);},_onTransactionClose:function NM__onTransactionClose(aHttpActivity)
...
{let mm=this.messageManager;mm.sendAsyncMessage("debug:netmonitor:"+this.connID+":newEvent",{id:this.id,event:event,});return this;}),};(function(){let methods=["addRequestHeaders","addRequestCookies","addRequestPostData","addResponseStart","addSecurityInfo","addResponseHeaders","addResponseCookies","addResponseContent","addEventTimings"];let factory=NetworkEventActorProxy.methodFactory;for(let method of methods){NetworkEventActorProxy.prototype[method]=factory(method);}})();function NetworkMonitorManager(frame,id)
