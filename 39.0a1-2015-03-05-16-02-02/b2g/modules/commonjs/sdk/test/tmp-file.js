"use strict";const{Cc,Ci,Cu}=require("chrome");const system=require("sdk/system");const file=require("sdk/io/file");const unload=require("sdk/system/unload");const tmpDir=require("sdk/system").pathFor("TmpD");const{Services}=Cu.import("resource://gre/modules/Services.jsm");let files=[];unload.when(function(){files.forEach(function(path){ try{if(file.exists(path))
file.remove(path);}
catch(e){console.exception(e);}});});
function readBinaryURI(uri){let ioservice=Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService);let channel=ioservice.newChannel2(uri,"UTF-8",null,null, Services.scriptSecurityManager.getSystemPrincipal(),null, Ci.nsILoadInfo.SEC_NORMAL,Ci.nsIContentPolicy.TYPE_OTHER);let stream=Cc["@mozilla.org/binaryinputstream;1"].createInstance(Ci.nsIBinaryInputStream);stream.setInputStream(channel.open());let data="";while(true){let available=stream.available();if(available<=0)
