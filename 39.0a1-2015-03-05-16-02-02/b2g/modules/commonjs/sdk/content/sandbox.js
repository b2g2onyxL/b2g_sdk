...
var unsafeWindowGetter=new content.Function('return window.wrappedJSObject || window;');Object.defineProperty(content,'unsafeWindow',{get:unsafeWindowGetter});let ContentWorker=load(content,CONTENT_WORKER_URL);let options='contentScriptOptions'in worker?JSON.stringify(worker.contentScriptOptions):undefined;
 ...
 let onEvent=Cu.exportFunction(onContentEvent.bind(null,this),ContentWorker);let chromeAPI=createChromeAPI(ContentWorker);let result=Cu.waiveXrays(ContentWorker).inject(content,chromeAPI,onEvent,options);
 model.emitToContent=result;let console=new PlainTextConsole(null,getInnerId(window));setListeners(this,console);if(requiresAddonGlobal(worker)){Object.defineProperty(getUnsafeWindow(window),'addon',{value:content.self,configurable:true});}
...
function createChromeAPI(scope){return Cu.cloneInto({timers:{setTimeout:timer.setTimeout.bind(timer),setInterval:timer.setInterval.bind(timer),clearTimeout:timer.clearTimeout.bind(timer),clearInterval:timer.clearInterval.bind(timer),},sandbox:{evaluate:evaluate,},},scope,{cloneFunctions:true});}