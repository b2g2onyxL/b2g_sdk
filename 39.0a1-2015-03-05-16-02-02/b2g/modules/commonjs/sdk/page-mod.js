...
return;for(let pagemod of pagemods){if(modMatchesURI(pagemod,document.URL))
onContent(pagemod,window);}}
function applyOnExistingDocuments(mod){getTabs().forEach(tab=>{ let window=getTabContentWindow(tab);if(!window||!window.frames)
return;let uri=getTabURI(tab);if(has(mod.attachTo,"top")&&modMatchesURI(mod,uri))
onContent(mod,window);if(has(mod.attachTo,"frame"))
getFrames(window).filter(iframe=>modMatchesURI(mod,iframe.location.href)).forEach(frame=>onContent(mod,frame));});}
...
onError:(e)=>emit(mod,'error',e)});workers.set(mod,worker);worker.on('*',(event,...args)=>{
 if(event==='attach')
...
return; let seen=modelFor(mod).seenDocuments;if(seen.has(window.document))
return;seen.set(window.document,true);let style=styleFor(mod);if(style)
attach(style,window);
...
return;domOff(window,eventName,onReady,true);createWorker(mod,window);
if(window.document.readyState=="complete"){mod.on('attach',worker=>{try{worker.send('pageshow');emit(worker,'pageshow');}
catch(e){}});}},true);}
