...
console.info("Error while loading localization: "+error.message);}).then(function onLocalizationReady(data){
 definePseudo(options.loader,'@l10n/data',data?data:null);return ready.then(()=>run(options,!!data));}).then(null,console.exception);return void 0;});function run(options,hasL10n){try{
try{

if(hasL10n&&options.main!=='sdk/test/runner'){require('../l10n/html').enable();}}
catch(error){console.exception(error);} 
if(preferences&&preferences.length>0){try{require('../preferences/native-options').enable({preferences:preferences,id:id}).catch(console.exception);}
catch(error){console.exception(error);}}
else{
...
if(get("extensions."+id+".sdk.debug.show",false)){BrowserToolboxProcess.init({addonID:id});}}catch(error){console.exception(error);throw error;}}
