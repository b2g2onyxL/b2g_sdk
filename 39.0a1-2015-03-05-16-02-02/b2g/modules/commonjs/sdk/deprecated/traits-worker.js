...
let self=this;timer.setTimeout(function(){self._emitToContent(JSON.stringify(array,replacer));},0);},emitSync:function emitSync(){let args=Array.slice(arguments);return this._emitToContent(Cu.cloneInto(args,this._addonWorker._window));},_onContentEvent:function onContentEvent(args){ let self=this;timer.setTimeout(function(){ self._emit.apply(self,JSON.parse(args));},0);},constructor:function WorkerSandbox(worker){this._addonWorker=worker;this.emit=this.emit.bind(this);this.emitSync=this.emitSync.bind(this); let window=worker._window;let proto=window;
...
var unsafeWindowGetter=new content.Function('return window.wrappedJSObject || window;');Object.defineProperty(content,'unsafeWindow',{get:unsafeWindowGetter});let ContentWorker=load(content,CONTENT_WORKER_URL);let options='contentScriptOptions'in worker?JSON.stringify(worker.contentScriptOptions):undefined;
 ...
 let chromeAPI=Cu.cloneInto({timers:{setTimeout:timer.setTimeout.bind(timer),setInterval:timer.setInterval.bind(timer),clearTimeout:timer.clearTimeout.bind(timer),clearInterval:timer.clearInterval.bind(timer),},sandbox:{evaluate:evaluate,},},ContentWorker,{cloneFunctions:true});let onEvent=Cu.exportFunction(this._onContentEvent.bind(this),ContentWorker);let result=Cu.waiveXrays(ContentWorker).inject(content,chromeAPI,onEvent,options);this._emitToContent=result;let self=this; this.on("console",function consoleListener(kind){console[kind].apply(console,Array.slice(arguments,1));}); this.on("message",function postMessage(data){if(self._addonWorker)
...
self._addonWorker._emit('error',error);}});if(worker._injectInDocument){let win=window.wrappedJSObject?window.wrappedJSObject:window;Object.defineProperty(win,"addon",{value:content.self,configurable:true});}
