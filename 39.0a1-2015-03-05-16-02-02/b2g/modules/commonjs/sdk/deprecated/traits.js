"use strict";module.metadata={"stability":"deprecated"};const{compose:_compose,override:_override,resolve:_resolve,trait:_trait,required,}=require('./traits/core');const{getOwnPropertyIdentifiers}=require('../util/object');const defineProperties=Object.defineProperties;const freeze=Object.freeze;const create=Object.create;function _create(proto,trait){let properties={},keys=getOwnPropertyIdentifiers(trait);for(let key of keys){let descriptor=trait[key];if(descriptor.required&&!Object.prototype.hasOwnProperty.call(proto,key))
...
function Public(instance,trait){let result={},keys=getOwnPropertyIdentifiers(trait);for(let key of keys){if(typeof key==='string'&&'_'===key.charAt(0)&&'__iterator__'!==key)
