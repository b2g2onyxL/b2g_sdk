"use strict";this.EXPORTED_SYMBOLS=["XulApp"];var{classes:Cc,interfaces:Ci}=Components;var exports={};this.XulApp=exports;var appInfo;
try{appInfo=Cc["@mozilla.org/xre/app-info;1"].getService(Ci.nsIXULAppInfo);}
catch(e){ appInfo={};}
var vc=Cc["@mozilla.org/xpcom/version-comparator;1"].getService(Ci.nsIVersionComparator);var ID=exports.ID=appInfo.ID;var name=exports.name=appInfo.name;var version=exports.version=appInfo.version;var platformVersion=exports.platformVersion=appInfo.platformVersion;


var ids=exports.ids={Firefox:"{ec8030f7-c20a-464f-9b0e-13a3a9e97384}",Mozilla:"{86c18b42-e466-45a9-ae7a-9b95ba6f5640}",SeaMonkey:"{92650c4d-4b8e-4d2a-b7eb-24ecf4f6b63a}",Fennec:"{aa3c5121-dab2-40e2-81ca-7ea25febc110}",Thunderbird:"{3550f703-e582-4d05-9a08-453d09bdfdc6}"};function is(name){if(!(name in ids))
...
exports.satisfiesVersion=satisfiesVersion;function incompatibility(module){let{metadata,id}=module;
if(!metadata||!("engines"in metadata))
return null;let{engines}=metadata;if(engines===null||typeof(engines)!=="object")
return new Error("Malformed engines' property in metadata");let applications=Object.keys(engines);let versionRange;applications.forEach(function(name){if(is(name)){versionRange=engines[name];

}});if(typeof(versionRange)==="string"){if(satisfiesVersion(versionRange))
