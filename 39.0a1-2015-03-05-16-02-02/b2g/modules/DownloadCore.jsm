...
if(this.error&&this.error.becauseBlockedByReputationCheck){return Promise.reject(new DownloadError({message:"Cannot start after being blocked "+"by a reputation check."}));}
this.stopped=false;this.canceled=false;this.error=null;this.hasProgress=false;this.hasBlockedData=false;this.progress=0;this.totalBytes=0;this.currentBytes=0;this.startTime=new Date();
...
if(this._promiseCanceled){try{yield OS.File.remove(this.target.path);}catch(ex){Cu.reportError(ex);}
this.target.exists=false;this.target.size=0;throw new DownloadError();}
...
if(this._currentAttempt==currentAttempt||!this._currentAttempt){if(!(ex instanceof DownloadError)){let properties={innerException:ex};if(ex.message){properties.message=ex.message;}
ex=new DownloadError(properties);}
this.error=ex;}
throw ex;}finally{this._saverExecuting=false;this._promiseCanceled=null;if(this._currentAttempt==currentAttempt||!this._currentAttempt){this._currentAttempt=null;this.stopped=true;this.speed=0;this._notifyChange();if(this.succeeded){yield this._succeed();}}}}.bind(this)));this._notifyChange();return currentAttempt;},_succeed:Task.async(function*(){yield DownloadIntegration.downloadDone(this);this._deferSucceeded.resolve();if(this.launchWhenSucceeded){this.launch().then(null,Cu.reportError);
if(this.source.isPrivate){gExternalAppLauncher.deleteTemporaryPrivateFileWhenPossible(new FileUtils.File(this.target.path));}else if(Services.prefs.getBoolPref("browser.helperApps.deleteTempFileOnExit")){gExternalAppLauncher.deleteTemporaryFileOnExit(new FileUtils.File(this.target.path));}}}),_promiseUnblock:null,_promiseConfirmBlock:null,unblock:function(){if(this._promiseUnblock){return this._promiseUnblock;}
if(this._promiseConfirmBlock){return Promise.reject(new Error("Download block has been confirmed, cannot unblock."));}
if(!this.hasBlockedData){return Promise.reject(new Error("unblock may only be called on Downloads with blocked data."));}
this._promiseUnblock=Task.spawn(function*(){try{yield OS.File.move(this.target.partFilePath,this.target.path);yield this.target.refresh();}catch(ex){yield this.refresh();this._promiseUnblock=null;throw ex;}
this.succeeded=true;this.hasBlockedData=false;this._notifyChange();yield this._succeed();}.bind(this));return this._promiseUnblock;},confirmBlock:function(){if(this._promiseConfirmBlock){return this._promiseConfirmBlock;}
if(this._promiseUnblock){return Promise.reject(new Error("Download is being unblocked, cannot confirmBlock."));}
if(!this.hasBlockedData){return Promise.reject(new Error("confirmBlock may only be called on Downloads with blocked data."));}
this._promiseConfirmBlock=Task.spawn(function*(){try{yield OS.File.remove(this.target.partFilePath);}catch(ex){yield this.refresh();this._promiseConfirmBlock=null;throw ex;}
this.hasBlockedData=false;this._notifyChange();}.bind(this));return this._promiseConfirmBlock;},launch:function(){if(!this.succeeded){return Promise.reject(new Error("launch can only be called if the download succeeded"));}
...
if(this.succeeded){let oldExists=this.target.exists;let oldSize=this.target.size;yield this.target.refresh();if(oldExists!=this.target.exists||oldSize!=this.target.size){this._notifyChange();}
return;}
if((this.hasPartialData||this.hasBlockedData)&&this.target.partFilePath){try{let stat=yield OS.File.stat(this.target.partFilePath);if(!this.stopped||this._finalized){return;}
this.currentBytes=stat.size;if(this.totalBytes>0){this.hasProgress=true;this.progress=Math.floor(this.currentBytes/this.totalBytes*100);}}catch(ex if ex instanceof OS.File.Error&&ex.becauseNoSuchFile){if(!this.stopped||this._finalized){return;}
this.hasBlockedData=false;this.hasPartialData=false;}
...
{let serializable={source:this.source.toSerializable(),target:this.target.toSerializable(),};let saver=this.saver.toSerializable();if(!saver){return null;}



if(saver!=="copy"){serializable.saver=saver;}
if(this.error){serializable.errorObj=this.error.toSerializable();}
if(this.startTime){serializable.startTime=this.startTime.toJSON();}
for(let property of kPlainSerializableDownloadProperties){if(this[property]){serializable[property]=this[property];}}
serializeUnknownProperties(this,serializable);return serializable;},getSerializationHash:function()
{

return this.stopped+","+this.totalBytes+","+this.hasPartialData+","+this.contentType;},};const kPlainSerializableDownloadProperties=["succeeded","canceled","totalBytes","hasPartialData","hasBlockedData","tryToKeepPartialData","launcherPath","launchWhenSucceeded","contentType",];Download.fromSerializable=function(aSerializable){let download=new Download();if(aSerializable.source instanceof DownloadSource){download.source=aSerializable.source;}else{download.source=DownloadSource.fromSerializable(aSerializable.source);}
...
if("errorObj"in aSerializable){download.error=DownloadError.fromSerializable(aSerializable.errorObj);}else if("error"in aSerializable){download.error=aSerializable.error;}
for(let property of kPlainSerializableDownloadProperties){if(property in aSerializable){download[property]=aSerializable[property];}}
deserializeUnknownProperties(download,aSerializable,property=>kPlainSerializableDownloadProperties.indexOf(property)==-1&&property!="startTime"&&property!="source"&&property!="target"&&property!="error"&&property!="saver");return download;}; this.DownloadSource=function(){}
...
this.DownloadTarget.prototype={path:null,partFilePath:null,exists:false,size:0,refresh:Task.async(function*(){try{this.size=(yield OS.File.stat(this.path)).size;this.exists=true;}catch(ex){if(!(ex instanceof OS.File.Error&&ex.becauseNoSuchFile)){Cu.reportError(ex);}
this.exists=false;}}),toSerializable:function()
...
if(aProperties.innerException){this.innerException=aProperties.innerException;}
this.stack=new Error().stack;}
this.DownloadError.prototype={__proto__:Error.prototype,result:false,becauseSourceFailed:false,becauseTargetFailed:false,becauseBlocked:false,becauseBlockedByParentalControls:false,becauseBlockedByReputationCheck:false,innerException:null,toSerializable:function()
{let serializable={result:this.result,message:this.message,becauseSourceFailed:this.becauseSourceFailed,becauseTargetFailed:this.becauseTargetFailed,becauseBlocked:this.becauseBlocked,becauseBlockedByParentalControls:this.becauseBlockedByParentalControls,becauseBlockedByReputationCheck:this.becauseBlockedByReputationCheck,};serializeUnknownProperties(this,serializable);return serializable;},};this.DownloadError.fromSerializable=function(aSerializable){let e=new DownloadError(aSerializable);deserializeUnknownProperties(e,aSerializable,property=>property!="result"&&property!="message"&&property!="becauseSourceFailed"&&property!="becauseTargetFailed"&&property!="becauseBlocked"&&property!="becauseBlockedByParentalControls"&&property!="becauseBlockedByReputationCheck");return e;}; this.DownloadSaver=function(){}
...
{throw new Error("Not implemented.");},}; this.DownloadSaver.fromSerializable=function(aSerializable){let serializable=isString(aSerializable)?{type:aSerializable}:aSerializable;let saver;switch(serializable.type){case"copy":saver=DownloadCopySaver.fromSerializable(serializable);break;case"legacy":saver=DownloadLegacySaver.fromSerializable(serializable);break;case"pdf":saver=DownloadPDFSaver.fromSerializable(serializable);break;default:throw new Error("Unrecoginzed download saver type.");}
...
let channel=NetUtil.newChannel2(download.source.url,null,null,null, Services.scriptSecurityManager.getSystemPrincipal(),null, Ci.nsILoadInfo.SEC_NORMAL,Ci.nsIContentPolicy.TYPE_OTHER);if(channel instanceof Ci.nsIPrivateBrowsingChannel){channel.setPrivate(download.source.isPrivate);}
...
if(Components.isSuccessCode(aStatusCode)){backgroundFileSaver.finish(Cr.NS_OK);}}}.bind(copySaver),onDataAvailable:function(aRequest,aContext,aInputStream,aOffset,aCount){backgroundFileSaver.onDataAvailable(aRequest,aContext,aInputStream,aOffset,aCount);}.bind(copySaver),},null);
...
yield deferSaveComplete.promise;yield this._checkReputationAndMove();}catch(ex){
...
throw ex;}}.bind(this));},_checkReputationAndMove:Task.async(function*(){let download=this.download;let targetPath=this.download.target.path;let partFilePath=this.download.target.partFilePath;if(yield DownloadIntegration.shouldBlockForReputationCheck(download)){download.progress=100;download.hasPartialData=false;


if(!DownloadIntegration.shouldKeepBlockedData()||!partFilePath){try{yield OS.File.remove(partFilePath||targetPath);}catch(ex){Cu.reportError(ex);}}else{download.hasBlockedData=true;}
throw new DownloadError({becauseBlockedByReputationCheck:true});}
if(partFilePath){yield OS.File.move(partFilePath,targetPath);}}),cancel:function DCS_cancel()
...
if(!this.download.target.partFilePath){try{let file=yield OS.File.open(this.download.target.path,{create:true});yield file.close();}catch(ex if ex instanceof OS.File.Error&&ex.becauseExists){}}
yield this._checkReputationAndMove();}catch(ex){
...
this.request=null;this.deferCanceled=null;this.firstExecutionFinished=true;}}.bind(this));},_checkReputationAndMove:function(){return DownloadCopySaver.prototype._checkReputationAndMove.call(this);},cancel:function DLS_cancel()
...
{this._redirects=redirects;},};this.DownloadLegacySaver.fromSerializable=function(){return new DownloadLegacySaver();}; this.DownloadPDFSaver=function(){}
this.DownloadPDFSaver.prototype={__proto__:DownloadSaver.prototype,_webBrowserPrint:null,execute:function(aSetProgressBytesFn,aSetPropertiesFn)
{return Task.spawn(function task_DCS_execute(){if(!this.download.source.windowRef){throw new DownloadError({message:"PDF saver must be passed an open window, and cannot be restarted.",becauseSourceFailed:true,});}
let win=this.download.source.windowRef.get();this.download.source.windowRef=null;if(!win){throw new DownloadError({message:"PDF saver can't save a window that has been closed.",becauseSourceFailed:true,});}
this.addToHistory();let targetPath=this.download.target.path;let file=yield OS.File.open(targetPath,{truncate:true});yield file.close();let printSettings=gPrintSettingsService.newPrintSettings;printSettings.printToFile=true;printSettings.outputFormat=Ci.nsIPrintSettings.kOutputFormatPDF;printSettings.toFileName=targetPath;printSettings.printSilent=true;printSettings.showPrintProgress=false;printSettings.printBGImages=true;printSettings.printBGColors=true;printSettings.printFrameType=Ci.nsIPrintSettings.kFramesAsIs;printSettings.headerStrCenter="";printSettings.headerStrLeft="";printSettings.headerStrRight="";printSettings.footerStrCenter="";printSettings.footerStrLeft="";printSettings.footerStrRight="";this._webBrowserPrint=win.QueryInterface(Ci.nsIInterfaceRequestor).getInterface(Ci.nsIWebBrowserPrint);try{yield new Promise((resolve,reject)=>{this._webBrowserPrint.print(printSettings,{onStateChange:function(webProgress,request,stateFlags,status){if(stateFlags&Ci.nsIWebProgressListener.STATE_STOP){if(!Components.isSuccessCode(status)){reject(new DownloadError({result:status,inferCause:true}));}else{resolve();}}},onProgressChange:function(webProgress,request,curSelfProgress,maxSelfProgress,curTotalProgress,maxTotalProgress){aSetProgressBytesFn(curTotalProgress,maxTotalProgress,false);},onLocationChange:function(){},onStatusChange:function(){},onSecurityChange:function(){},});});}finally{ this._webBrowserPrint=null;}
let fileInfo=yield OS.File.stat(targetPath);aSetProgressBytesFn(fileInfo.size,fileInfo.size,false);}.bind(this));},cancel:function DCS_cancel()
{if(this._webBrowserPrint){this._webBrowserPrint.cancel();this._webBrowserPrint=null;}},toSerializable:function()
{if(this.download.succeeded){return DownloadCopySaver.prototype.toSerializable.call(this);}


return null;},};this.DownloadPDFSaver.fromSerializable=function(aSerializable){return new DownloadPDFSaver();};