const EXPORTED_SYMBOLS=["httpRequest","percentEncode"];const{classes:Cc,interfaces:Ci}=Components;
...
if(aOptions.onLoad)
aOptions.onLoad(target.responseText,this);}catch(e){if(aOptions.onError)
...
let POSTData=aOptions.postData||null;if(POSTData&&Array.isArray(POSTData)){xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded; charset=utf-8");POSTData=POSTData.map(function(p)p[0]+"="+percentEncode(p[1])).join("&");}
