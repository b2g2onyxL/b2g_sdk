...
try{directoryPath=this._getDirectory("DfltDwnld");}catch(e){directoryPath=yield this._createDownloadsDirectory("Home");}
this._downloadsDirectory=directoryPath;throw new Task.Result(this._downloadsDirectory);}.bind(this));},_downloadsDirectory:null,getPreferredDownloadsDirectory:function DI_getPreferredDownloadsDirectory(){return Task.spawn(function(){let directoryPath=null;let prefValue=1;try{prefValue=Services.prefs.getIntPref("browser.download.folderList");}catch(e){}
...
throw new Task.Result(directoryPath);}.bind(this));},getTemporaryDownloadsDirectory:function DI_getTemporaryDownloadsDirectory(){return Task.spawn(function(){let directoryPath=null;
if(Services.metro&&Services.metro.immersive){directoryPath=yield this.getSystemDownloadsDirectory();}else{directoryPath=this._getDirectory("TmpD");}
throw new Task.Result(directoryPath);}.bind(this));},shouldBlockForParentalControls:function DI_shouldBlockForParentalControls(aDownload){if(this.dontCheckParentalControls){return Promise.resolve(this.shouldBlockInTest);}
...
yield OS.File.setPermissions(aDownload.target.path,{unixMode});}catch(ex){
