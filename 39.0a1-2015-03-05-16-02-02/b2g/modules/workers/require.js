...
let require=(function(){let modules=new Map();Object.defineProperty(Error.prototype,"moduleStack",{get:function(){return this.stack;}});Object.defineProperty(Error.prototype,"moduleName",{get:function(){let match=this.stack.match(/\@(.*):.*:/);if(match){return match[1];}
return"(unknown module)";}});return function require(path){if(typeof path!="string"||path.indexOf("://")==-1){throw new TypeError("The argument to require() must be a string uri, got "+path);} 
...
let code=new Function("exports","require","module",source+"\n//# sourceURL="+uri+"\n");code(exports,require,module);}catch(ex){
modules.delete(path);throw ex;}
