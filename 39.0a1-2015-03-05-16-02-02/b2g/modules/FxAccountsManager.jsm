...
return this._fxAccounts.setSignedInUser(user).then(()=>{this._activeSession=user;log.debug("User signed in: "+JSON.stringify(this._user)+" - Account created "+(aMethod=="signUp"));return Promise.resolve({accountCreated:aMethod==="signUp",user:this._user});});},reason=>{return this._serverError(reason);});},_handleGetAssertionError:function(reason,aAudience,aPrincipal){log.debug("FxAccountsManager._handleGetAssertionError()");let errno=(reason?reason.errno:NaN)||NaN;if(errno==ERRNO_INVALID_AUTH_TOKEN){return this._fxAccounts.accountStatus().then((exists)=>{
if(exists){return this.getAccount().then((user)=>{return this._refreshAuthentication(aAudience,user.email,aPrincipal,true );});} 
return this._localSignOut().then(()=>{return this._uiRequest(UI_REQUEST_SIGN_IN_FLOW,aAudience,aPrincipal);},(reason)=>{ log.error("Signing out in response to server error threw: "+
...
log.debug("Signed out");return Promise.resolve();},reason=>{return this._serverError(reason);});});},_uiRequest:function(aRequest,aAudience,aPrincipal,aParams){if(Services.io.offline){return this._error(ERROR_OFFLINE);}
...
return this._signOut();},resendVerificationEmail:function(){return this._fxAccounts.resendVerificationEmail().then((result)=>{return result;},(error)=>{return this._error(ERROR_SERVER_ERROR,error);});},getAccount:function(){if(this._activeSession){if(!this._activeSession.verified){this.verificationStatus(this._activeSession);}
...
this._activeSession=user;if(!user.verified){this.verificationStatus(user);}
...
if(Services.io.offline){log.warn("Offline; skipping verification.");return;}
...
let secMan=Cc["@mozilla.org/scriptsecuritymanager;1"].getService(Ci.nsIScriptSecurityManager);let uri=Services.io.newURI(aPrincipal.origin,null,null);log.debug("FxAccountsManager.getAssertion() aPrincipal: ",aPrincipal.origin,aPrincipal.appId,aPrincipal.isInBrowserElement);let principal=secMan.getAppCodebasePrincipal(uri,aPrincipal.appId,aPrincipal.isInBrowserElement);return this.getAccount().then(user=>{if(user){if(!user.verified){return this._error(ERROR_UNVERIFIED_ACCOUNT,{user:user});}
...
let permission=permissionManager.testPermissionFromPrincipal(principal,FXACCOUNTS_PERMISSION);if(permission==Ci.nsIPermissionManager.PROMPT_ACTION&&!this._refreshing){return this._refreshAuthentication(aAudience,user.email,principal,false );}else if(permission==Ci.nsIPermissionManager.DENY_ACTION&&!this._refreshing){return this._error(ERROR_PERMISSION_DENIED);}else if(this._refreshing){
